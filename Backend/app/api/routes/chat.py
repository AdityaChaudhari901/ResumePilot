...
def openclaw_command(
    ...
    resume = latest_resume(db, current_user)
    if not resume:
        raise HTTPException(
    ...
    response = analyze_job(db, analysis_request, current_user, settings)
    markdown = get_report_markdown(db, response.report_id, current_user)
