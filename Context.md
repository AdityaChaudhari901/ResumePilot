...
Simplified evidence report browser-verified (2026-07-11):

- Reorganized the dedicated application/report surfaces around a decision-first brief: evidence-fit score and provenance, executive summary, matched/missing/weak/warning counts, then four compact at-a-glance tiles for strengths, risks, ATS keywords, and the next three actions.
- Moved score calculations, the complete skill ledger, resume recommendations, cover letter, and interview preparation into accessible native disclosures. No report array is silently truncated: every matched skill, missing or weak requirement, tailored bullet, ATS keyword, next action, evidence reference, and interview group remains available on demand.
- Kept validation warnings, score authority, report identity, Markdown export, application-linked tailoring gate, workflow mode/status, historical/provisional labels, and the deterministic-score disclaimer visible or contract-preserved. OpenClaw, backend APIs, database state, LangGraph approval ownership, and LangChain confinement are unchanged.
- Replaced repeated nested cards with quiet divided evidence lists, collapsed per-item source references, a collapsed full cover letter, and category-level interview disclosures. The report now uses the bento grid only for the high-level decision surface.
- Replaced the confusing `Project/work evidence strength` percentage with `Work/project proof`: a direct backed count, an accurate count sourced from other resume sections, and an explicit note that the diagnostic does not change the fit score. Its full source list remains collapsed behind a plain-language explanation.
- Added a separate `Open cover letter` action in the report header. The report already creates and persists this draft during analysis, so the truthful view action opens the evidence-backed draft and its application-materials section, focuses the native disclosure summary, respects reduced-motion preferences, and does not trigger another AI run or consume additional quota.
- Reduced the tested initial report from 5,955 px to 1,789 px at 1440 px and from 15,649 px to 3,978 px at 320 px. The fully expanded 320 px evidence state remains horizontally contained, keyboard operable, and WCAG A/AA clean, including schema-unbounded skill and keyword tokens with no natural breakpoints.
- Release evidence is green: npm audit reports 0 vulnerabilities; ESLint, TypeScript, auth-runtime checks, and the Next.js production build pass; focused desktop/mobile report coverage passes 2/2 and the complete production Chromium suite passes 19/19.
- Rebuilt and recreated only the production-like frontend, then verified the real saved `AI Native Engineer` report at `http://127.0.0.1:3050/app/applications/2/report`. Light/dark desktop and 320 px dark collapsed/expanded scans have zero WCAG A/AA violations, horizontal overflow, console/page/HTTP errors, or actionable request failures; direct cover-letter focus and the `5 backed / 8 other sections` diagnostic copy are correct.

Completed application bento overview browser-verified (2026-07-11):

- Replaced the completed `/app/applications/[applicationId]` inline report stack with a compact semantic case overview. The dedicated `/report` route remains the only full fit-analysis surface, and the dedicated `/resume` route remains the only approval/edit/export surface.
- Added an asymmetric six-tile evidence bento for the next decision, versioned evidence-fit score, evidence snapshot, six-stage proof chain, bounded report preview, and tailored-resume approval status. Each tile communicates its evidence state and allowed action without duplicating the score breakdown, cover letter, interview preparation, or full resume review.
- Preserved incomplete-case workflow behavior, protected route hydration, active-operation provenance, score/validation authority, evidence-linked export rules, application status, and the collapsed parsed-resume evidence review. No backend, database, OpenClaw, LangGraph, or LangChain boundary changed.
- Added desktop geometry, route-separation, WCAG A/AA, 320 px stacking, and no-horizontal-overflow regressions to the production browser flow. The first Axe pass caught and drove fixes for inverse-card contrast and description-list semantics.
- Release evidence is green: npm audit reports 0 vulnerabilities; ESLint, TypeScript, auth-runtime checks, and the production build pass; the focused desktop/mobile flow passes 2/2 and the complete Chromium suite passes 19/19 with the compact case overview visually reviewed at 1440 px and 320 px.
- Rebuilt only the production-like frontend container and verified the real saved `AI Native Engineer` case at `http://127.0.0.1:3050/app/applications/2` in dark mode. The stack is healthy/ready at Alembic `20260711_0011`; the exact live 320 px render has `scrollWidth=clientWidth=320` and zero WCAG A/AA Axe violations.
...
- Added `evidence_v2` with required skills weighted at 50, responsibilities at 20, preferred skills at 10, explicit tenure at 15, and domain evidence at 5. Not-applicable dimensions are removed and reweighted; unknown dimensions retain their effective weight at zero contribution, so missing information cannot increase the score and makes it provisional.
- Candidate experience now requires candidate-attributed resume tenure and a required job minimum. Preferred, company, team, client, mentee, object, conflicting, word-number-ambiguous, and upper-bound-only job tenure cannot become a scored minimum. Responsibility matching uses exact canonical tokens, zero tenure gets zero credit, and persisted caps explain scores limited by missing required skills or actionable evidence.
- Analyses, applications, and queued jobs persist score version/status provenance. Queued work snapshots its scorer, historical rows are classified as `deterministic_v1` or `legacy_unversioned`, old scoring remains executable, worker startup verifies migration readiness, and PostgreSQL fences incompatible old workers from `evidence_v2` jobs.
- Kept stored report/match JSON in its pre-versioning shape for binary rollback. Relational metadata hydrates read boundaries; finalization rejects mismatched totals, versions, statuses, or breakdowns; permanent score-contract corruption fails without retry churn.
- Added 38 labeled score cases with 14 monotonic pairwise expectations, guarded provenance-preserving rollback, typed API/UI breakdowns, evidence references, historical version labels in report/ledger/application surfaces, and responsive accessible progress indicators. OpenClaw, LangGraph approval/checkpoint ownership, and LangChain confinement to LangGraph nodes remain unchanged.
- Release evidence is green: 220 backend tests at 88.90% coverage, Ruff/compile, 20-pair and 38-case score gates, SQLite and PostgreSQL 16 migration round trips, frontend audit/lint/type/build, 13 Chromium tests, and 7 OpenClaw tests.
...
- Implemented application structure:
  ...
  - `Backend/app/services/score_contract_service.py`
  ...
  - `Backend/evals/match_score_cases.json`
  ...
  - `Frontend/src/features/dashboard/components/match-score-breakdown.tsx`
...
| Simplified evidence-report gate | `npm run test:e2e -- --grep "dashboard demo flow"`, 1440/320 screenshot inspection, then full `npm run test:e2e` | Passed: focused report flow 2/2 and complete production Chromium suite 19/19; collapsed and expanded WCAG A/AA scans, Enter/Space disclosure controls, direct cover-letter reveal/focus, plain-language work/project proof, full oversized report arrays, score/validation/export/history contracts, 320 px expanded-state overflow with 300+ character unbroken values, and standalone application boundaries all verified. Initial height fell from 5,955 px to 1,789 px at 1440 and from 15,649 px to 3,978 px at 320 without hiding data permanently |
| Live simplified report render | Frontend-only Compose rebuild/recreate, `/health`, `/ready`, saved report HTTP probe, then light/dark 1440 and dark 320 collapsed/expanded Playwright/Axe checks | Passed: frontend/backend/database healthy; readiness confirms Alembic `20260711_0011` and LangGraph checkpoint schema; saved report returned 200; desktop height is 1,737 px; collapsed 320 height is 4,128 px and fully expanded height is 16,764 px with `scrollWidth=clientWidth=320`; both themes and both disclosure states have zero WCAG A/AA violations, console/page/HTTP errors, or actionable request failures; cover-letter focus and `5 backed by work/projects · 8 from other resume sections · Does not change the fit score` verified |
| Completed application bento gate | `npm run test:e2e -- --grep "dashboard demo flow"`, authority-edge focused run, then the full `npm run test:e2e` | Passed: focused desktop/mobile flow 2/2, historical/draft/source authority regression 1/1, and full production Chromium suite 19/19; dedicated report/resume route separation, versioned/provisional/block score labels, fail-closed draft ownership, route-hydrated source provenance, 1440 px asymmetric placement, 320 px stacking, zero horizontal overflow, and WCAG A/AA all verified; desktop/mobile screenshots visually reviewed |
| Live completed-case bento render | Frontend-only Compose rebuild/recreate, `/health`, `/ready`, application HTTP probe, 1440/320 dark screenshots, and live 320 px Axe/overflow probe | Passed: frontend/backend/database healthy, readiness confirms Alembic `20260711_0011` plus LangGraph checkpoint schema, application route returned 200, real `AI Native Engineer` case rendered the bounded six-tile overview, `scrollWidth=clientWidth=320`, and Axe returned zero WCAG A/AA violations |
...
- Simplified the full evidence report into a decision brief, four-tile summary, and accessible progressive-disclosure sections; preserved complete evidence and report authority, exposed readable risk/keyword statuses, replaced the blended work/project diagnostic percentage with backed/other-section counts, added a separate cover-letter action, removed silent array truncation, hardened unbroken-token wrapping, added keyboard/Axe/expanded-mobile regressions, and verified the full 19-test browser suite.
- Replaced the completed application route's duplicated full report/sidebar with a responsive six-tile case-file bento; retained full report evidence and tailored-resume approval/export on their dedicated routes, centralized versioned score labels, fail-closed unverified draft/source states, added desktop/mobile layout and Axe regressions, and verified the complete 19-test browser suite.
